...
public class FakeLeaveCommand implements CommandExecutor, TabCompleter {
    ...
    public FakeLeaveCommand(JavaPlugin plugin) {
        this.plugin = plugin;
    }

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission("fakeleave.use")) {
            sender.sendMessage(ChatColor.RED + "You don't have permission to use this command.");
            return true;
        }

        if (args.length < 1) {
            sender.sendMessage(ChatColor.YELLOW + "Usage: /fakeleave <playerName> [custom message]");
            return true;
        }
        ...
        plugin.getLogger().info(sender.getName() + " sent fake leave: " + message);
        return true;
    }
